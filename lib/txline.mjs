...
// Server-side match history — every finished fixture in the current TxLINE
// window, with scorers/minutes already enriched by getScore. Served from the
// backend so every device sees the same history (localStorage only ever holds
// matches a given browser witnessed finishing live).
export async function getHistory() {
  const { source, fixtures } = await getFixtures()
  const settled = await Promise.all(
    fixtures.map(async (f) => {
      if (f.id.startsWith('wc-')) return null // skip the demo fixture
      try {
        const { score } = await getScore(f.id)
        if (!score || score.status !== 'finished') return null
        return { ...f, status: 'finished', score }
      } catch {
        return null
      }
    }),
