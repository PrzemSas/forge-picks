...
export async function getTournament() {
  ...
  // TheSportsDB's timelines are partial — it currently names about half the
  // goals — so the UI says so once, above the list, rather than per match.
  // Timelines are cached per event, so a warm tournament view costs nothing.
  await Promise.all(
    matches
