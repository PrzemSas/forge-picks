...
// TheSportsDB's intRound codes. Their documented cup scheme is 125=quarter,
// 150=semi, 160/170/180=playoffs, 200=final — that's what the World Cup feed
// actually sends. The small numbers are the "teams remaining" style other
// leagues use, kept because they cost nothing and some competitions do send
// them. Anything unmapped falls back to "Round N" and sorts last.
const ROUND_LABEL = {
  1: 'Group stage',
  16: 'Round of 16',
  8: 'Quarter-final',
...
export async function getTournament() {
  ...
  // Goal scorers for played matches, from the same timeline the live path uses.
  // TheSportsDB's timelines are partial — a 4–1 often names only three of the
  // five — so we also count how many goals went unnamed instead of quietly
  // rendering a short list that reads like a bug. Timelines are cached per
  // event, so a warm tournament view costs nothing extra.
  await Promise.all(
    matches
      .filter((m) => m.status === 'finished')
      .map(async (m) => {
        const tl = await eventScorers(m.id)
        m.scorers = tl.map((g) => ({ name: g.name, minute: g.minute, home: g.tsdbHome }))
        const goals = (m.homeScore ?? 0) + (m.awayScore ?? 0)
        m.unnamedGoals = Math.max(0, goals - m.scorers.length)
      }),
  )
